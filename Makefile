...
WASM_BUILD := $(WASM_BUILD_DIR)/lndmobile.wasm
WASM_EXEC_BUILD := $(WASM_BUILD_DIR)/wasm_exec.js
...
#? gen-wasm-registry: Generate wasm RPC registries from mobile generated bindings
gen-wasm-registry:
	@$(call print, "Generating wasm RPC registries.")
	$(GOCC) run ./cmd/genwasmregistry -tags "mobile $(DEV_TAGS) $(RPC_TAGS)"

#? wasm: Build the browser wasm bundle into wasm/build
wasm: mobile-rpc mobile-gomobile-mode gen-wasm-registry
	@$(call print, "Building browser wasm bundle ($(WASM_BUILD)).")
...
#? clean-wasm: Remove browser wasm build artifacts
clean-wasm:
	@$(call print, "Cleaning wasm build artifacts.")
	$(RM) -r wasm/build
