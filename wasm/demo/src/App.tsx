...
function App() {
  ...
  const subscribeStateHandleRef = useRef<ReturnType<
    typeof openServerStream
  > | null>(null);
  ...
  async function stopDaemon() {
    ...
    subscribeStateHandleRef.current = null;
  ...
  async function startSubscribeState() {
    if (subscribeStateHandleRef.current) {
    ...
    subscribeStateHandleRef.current = handle;
    setStateStreamActive(true);
    return { subscribed: true };
  }

  async function stopSubscribeState() {
    const handle = subscribeStateHandleRef.current;
    if (!handle) {
    ...
    subscribeStateHandleRef.current = null;
    setStateStreamActive(false);
    appendLog("subscribe_state stopped");
    return { subscribed: false };
  }
        ...
        <section className="panel">
          ...
          <div className="button-grid">
            ...
            <button
              className="alt"
              onClick={() =>
                void runAction("subscribe_state_stop", stopSubscribeState)
              }
            >
              Stop SubscribeState
            </button>
