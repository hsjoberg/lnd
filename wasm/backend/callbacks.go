...
// Shared JS <-> Go callback glue used by the backend RPC bridge. The mobile
// bindings use Go callback interfaces, while the browser side uses JS
// functions and Uint8Array values.
...
func bytesFromJS(v js.Value) ([]byte, error) {
	...
	if uint8ArrayCtor.IsUndefined() || !v.InstanceOf(uint8ArrayCtor) {
		return nil, errors.New("expected Uint8Array value")
	}
...
func newJSStreamHandle(send func([]byte) error, stop func() error) js.Value {
	...
	stopFn = js.FuncOf(func(_ js.Value, _ []js.Value) any {
		stopOnce.Do(func() {
			// After stop, this handle should not receive further traffic from JS.
			obj.Set("send", js.Undefined())
			obj.Set("stop", js.Undefined())

			if err := stop(); err != nil {
				stopResult = err.Error()
			}
			releaseFuncs()
		})
		return stopResult
	...
	obj.Set("stop", stopFn)
	if send != nil {
		releaseFuncs = retainStreamJSFuncs(sendFn, stopFn)
	} else {
		releaseFuncs = retainStreamJSFuncs(stopFn)
	}
