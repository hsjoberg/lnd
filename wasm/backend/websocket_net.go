...
func newWebsocketConn(url, alias string, timeout time.Duration) (*websocketConn, error) {
	...
	ws := wsCtor.New(url, js.ValueOf([]any{"binary", "base64"}))
...
func (c *websocketConn) Write(p []byte) (int, error) {
	...
	defer func() {
		if r := recover(); r != nil {
			c.setCloseErr(fmt.Errorf("websocket send failed: %v", r))
			c.close()
		}
	}()
...
func (c *websocketConn) RemoteAddr() net.Addr {
	addr, err := net.ResolveTCPAddr("tcp", c.alias)
	if err == nil {
		return addr
	}
...
func (n *websocketNet) Dial(network, address string, timeout time.Duration) (net.Conn, error) {
	...
	return newWebsocketConn(wsURL, address, timeout)
}
